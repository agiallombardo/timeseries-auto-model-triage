...
def grid_search_lstm_features(X_train, X_test, y_train, y_test, loss='l2', results_dir=None,
    ...
    if results:
        path = (os.path.join(results_dir, 'lstm_feat_grid_search_results.csv')
                if results_dir else 'lstm_feat_grid_search_results.csv')
    ...
    if best_params is not None:
        ...
        model.add(LSTM(best_params['units'], activation=act))
