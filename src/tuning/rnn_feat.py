...
def grid_search_rnn_features(X_train, X_test, y_train, y_test, loss='l2', results_dir=None,
    ...
    """
    logger.info(f"Performing grid search for RNN-feat ({loss.upper()}, scaler={scaler}) model...")
    n_steps_list = [3, 5, 7]
    units_list = [32, 64]
    ...
    for (n_steps, act, units, dr, lr) in tqdm(
        combos, desc=f"RNN-feat ({loss}) grid", unit="candidate", leave=False
    ):
        ...
        try:
            ...
            model.add(SimpleRNN(units, activation=act))
            if dr > 0:
            ...
            logger.warning("Error training RNN-feat: %s", e)
            continue

    logger.info("Best RNN-feat parameters: %s with RMSE: %.4f", best_params, best_rmse)
    if results:
        path = (os.path.join(results_dir, 'rnn_feat_grid_search_results.csv')
                if results_dir else 'rnn_feat_grid_search_results.csv')
    ...
    if best_params is not None:
        ...
        model.add(SimpleRNN(best_params['units'], activation=act))
