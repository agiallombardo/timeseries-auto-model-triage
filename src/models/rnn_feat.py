...
def run_rnn_features(X_train, X_test, y_train, y_test, n_steps=5, loss='l2',
                     scaler='standard', feature_range=(0, 1),
                     activation='relu', learning_rate=0.001, **kwargs):
    """Many-to-one RNN on feature matrices; one forward pass per test sample."""
    logger.info(f"Training RNN-feat ({loss.upper()}, scaler={scaler}, act={activation}) model...")
